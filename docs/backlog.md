...
| OC-9 | **First round-trip build to both stores** | 🔄 2026-08-10: **iOS done end-to-end** — build succeeded, submitted to App Store Connect (API key, **App Manager** role — least privilege, not Admin), Apple processing it for TestFlight now. Once processed it should attach to the "OverTesters" group automatically (`usesNonExemptEncryption: false` already answers export compliance) — Matías installs via the TestFlight app on his own iPhone to close the loop. **Android build also done** (new EAS-managed keystore, `.aab` artifact ready) but **submit is blocked on OC-7** (Play Developer account not yet verified) — `eas submit --platform android --latest` once that clears. Phase-0 exit criterion is met for iOS; Android pending only on the Play account. | 🔄 |
