...
| OC-9 | **First round-trip build to both stores** | 🔄 2026-08-10: iOS build succeeded (`eas build --profile production --platform ios`), submit to TestFlight in progress (`eas submit --platform ios --latest`, App Store Connect API key created with **App Manager** role — least privilege, not Admin). Android build in progress (`--platform android`, new EAS-managed keystore). **Android submit blocked on OC-7** (Play Developer account not yet verified) — will run `eas submit --platform android --latest` once that clears. This is the phase-0 exit criterion. | 🔄 |
