...
|---|---|---|---|
| OC-1 | Install **Xcode 26** + iOS 26 simulator runtimes | ⚠️ **Only Command Line Tools are installed today** (`xcode-select -p` → `/Library/Developer/CommandLineTools`). ~40 GB; ~142 GB free. Also `sudo xcodebuild -license accept`. ⚠️ **Xcode 26 / iOS 26 SDK has been mandatory for every App Store Connect upload since 2026-04-28** — this is not optional even if EAS builds in the cloud. | ⬜ |
| OC-2 | Install Android Studio + SDK **API 36** + an emulator image | ⚠️ No `~/Library/Android/sdk` today. ⚠️ System JDK is **26** (Homebrew) — React Native's Gradle setup expects **JDK 17 or 21**; install one and pin `JAVA_HOME` per-project rather than changing the system default. ⚠️ Play requires target API 36 for all uploads from **2026-08-31**. | ⬜ |
