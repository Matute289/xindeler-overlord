...
| ID | Item | Notes | Status |
|---|---|---|---|
| OC-10 | Theme + design tokens | ✅ Done 2026-08-10. `src/ui/theme.ts` (dark-first colors, spacing, typography, `fonts`) mirrored by hex in `tailwind.config.js`; light is a courtesy fallback via `prefers-color-scheme`/`useColorScheme`, not the design. Inter (400/600/700) loaded via `@expo-google-fonts/inter` and applied through the new `fonts` token map. Two primitives shipped so far — `Screen`, `Empty` — the other ~13 from spec §3 are deliberately deferred to the screens that need them (Phase 1+). App icon, splash icon, Android adaptive icon and web favicon replaced with the corrected full-bleed Overlord crest (verified in the production web export's generated `favicon.ico`, not just the source PNG). The O-mark loading animation is deliberately deferred, no screen needs it yet. | ✅ |
| OC-11 | Navigation skeleton | ✅ Done 2026-08-10. `app/(tabs)/_layout.tsx`: one route tree, 5 destinations (Status·Jugadores·Logs·ORACLE·Más), bottom tabs under 768 pt width, persistent left sidebar at/above it (`useBreakpoint` hook, `useWindowDimensions`-driven, live-reactive with no reload). Manual verification pass (Task 8) on iPhone sim, iPad sim, and web found and fixed one real bug: the bottom tab bar was rendering with React Navigation's untouched default theme (white bar, iOS-blue active tint) instead of the app's dark palette — fixed by wiring `tabBarStyle`/`tabBarActiveTintColor`/`tabBarInactiveTintColor` from `colors.*`, matching the sidebar. Sidebar's active-item highlight (`usePathname() === '/'` for Status at the root route) is still only code-reviewed, not runtime-verified — the verification environment couldn't drive either the iPad simulator or the automated browser past the 768pt threshold (iPadOS 26's windowed-app/Stage Manager default on the M5 iPad sim, and a fixed/unresponsive viewport in the browser automation tool); see `task-8-report.md` for the full account. | ✅ |
